...
class MyProductController extends Controller
{
    ...
    public function postEditProduct(Request $request, $id)
    {
        ...
        if ($request->hasFile('avatar')) {
            ...
            if (count($newAvatarData) > 3) {
                return redirect('/account/add-product')->withErrors(['Lỗi! Chỉ được thêm 3 hình ảnh!']);
            }
        ...
        $product->update($data);
        return redirect('/account/my-product')->with('success', 'Updated product successfully');
    }
    ...
    public function postProduct(ProductRequest $request)
    {
        ...
        if ($product) {
            return redirect('/account/my-product')->with('success', 'Add product success.');
        } else {
            return redirect('/account/my-product')->withErrors('Add product error.');
        }
    }
